...
let getCQArr = ins => {
  return [
    ...
    { xy: [488, 309], key: false, mouseKey: 'right', checkBefore: 'djl' }, //打开飞行符
    {
      ...
      ofset: [5, 5],
      checkAfter: 'zzffIcon',
      checkBefore: 'zzffIcon',
      type: 'notfont'
    }, //前往宝象国 关闭物品栏 并判断是否已经完成飞行;
    {
      xy: [440, 370],
      beforeKey: ['f9'],
      key: ['e', 'alt'],
...
global.robotFun = () => {
  ...
  let x = zzArr[zzIndex].xy[0] + global.windowPixel.x,
    y = zzArr[zzIndex].xy[1] + global.windowPixel.y
...
ioHook.on('keydown', event => {
  ...
  if (event.keycode == 60) {
    ...
    setInterval(() => {
      ...
      robot.keyTap('tab')
      func.getPx(
        [global.icons.iszd, global.icons.d4icon, global.icons.zidongzd],
        (a, b) => {
          if (a[1] && a[1].x > 408) {
            robotAction(
              [381 + global.windowPixel.x, 180 + global.windowPixel.y],
              'left',
              false,
              () => {
                robot.keyTap('tab')
                robot.moveMouse(
                  40 + global.windowPixel.x,
                  40 + global.windowPixel.y
                )
              }
            )
          ...
          if (a[1] && a[1].x < 408) {
            robotAction(
              [440 + global.windowPixel.x, 180 + global.windowPixel.y],
              'left',
              false,
              () => {
                robot.keyTap('tab')
            ...
            parseInt(
              robot.getPixelColor(
                global.windowPixel.x + 70,
                global.windowPixel.y - 10
              ),
              16
            ) == 16762263 ||
            parseInt(
              robot.getPixelColor(
                global.windowPixel.x + 194,
                global.windowPixel.y - 10
              ),
              16
            ) == 16762263 ||
